...
GhostCells::GhostCells(const std::string &name, int moduleID, mpi::communicator comm): Module(name, moduleID, comm)
{
...
void createVertices(StructuredGridBase::const_ptr grid, typename Connected::ptr conn, GhostCells::DataMapping &vm)
{
    vm.clear();
...
bool GhostCells::compute(const std::shared_ptr<BlockTask> &task) const
{
    //GhostCells Polygon
    ...
    std::unique_lock<std::mutex> guard(m_blockDataMutex);
    auto &bd = m_blockData[ugrid->getBlock()];
    m_blockDataMutex.unlock();
    bd.grid = ugrid;
    bd.boundaryVertices = boundaryVertices(ugrid);
    auto bounds = ugrid->getBounds();
    bd.min = bounds.first;
    bd.max = bounds.second;
    return true;
...
GhostCells::Result<Quads> GhostCells::createSurface(vistle::StructuredGridBase::const_ptr grid, bool haveElementData,
                                                    bool createSurf, bool createLines) const
{
...
void GhostCells::renumberVertices(Coords::const_ptr coords, Indexed::ptr poly, DataMapping &vm) const
{
...
void GhostCells::renumberVertices(Coords::const_ptr coords, Quads::ptr quad, DataMapping &vm) const
{
    const bool reuseCoord = getIntParameter("reuseCoordinates");
...
GhostCells::Result<Polygons> GhostCells::createSurface(vistle::UnstructuredGrid::const_ptr m_grid_in,
                                                       bool haveElementData, bool createSurface, bool createLines) const
{
...
std::vector<Index> GhostCells::boundaryVertices(vistle::UnstructuredGrid::const_ptr m_grid_in) const
{
    const bool showgho = getIntParameter("ghost");
    const bool showtet = getIntParameter("tetrahedron");
    const bool showpyr = getIntParameter("pyramid");
    ...
    std::vector<unsigned char> vertexUsed(m_grid_in->getNumVertices(), 0);
    ...
    for (Index i = 0; i < vertexUsed.size(); ++i) {
        if (vertexUsed[i])
            result.push_back(i);
    }
...
//bool GhostCells::checkNormal(Index v1, Index v2, Index v3, Scalar x_center, Scalar y_center, Scalar z_center) {
...
struct Bounds {
    ...
    Bounds() {}
    Bounds(int rank, int block, vistle::Vector3 min, vistle::Vector3 max): rank(rank), block(block), min(min), max(max)
    {}

    template<class Archive>
    void serialize(Archive &ar, int ver)
    {
        ar &rank;
        ar &block;
        ar &min;
...
struct ExchangeSize {
    ...
    ExchangeSize() {}
    ExchangeSize(int block, Index nvert): block(block), nvert(nvert) {}
...
bool GhostCells::reduce(int t)
{
    std::vector<Bounds> localBounds;

    for (auto &b: m_blockData) {
        localBounds.emplace_back(rank(), b.first, b.second.min, b.second.max);
    }

    std::vector<Bounds> allBounds;
    mpi::all_gather(comm(), localBounds.data(), localBounds.size(), allBounds);

    auto inside = [](const Vector3 &c, const Vector3 &min, const Vector3 &max) -> bool {
    ...
    std::vector<std::vector<Index>> exchangeSize(size());
    for (auto &b: m_blockData) {
        auto &candidateVertices = b.second.candidateVertices;
        candidateVertices.resize(allBounds.size());
        auto &grid = b.second.grid;
        auto nvert = grid->getNumVertices();
        ...
        for (Index i = 0; i < nvert; ++i) {
            Index v = cl[i];
            Vector3 c(x[v], y[v], z[v]);
            for (Index blockIdx = 0; blockIdx < allBounds.size(); ++blockIdx) {
                auto &bo = allBounds[blockIdx];
                if (bo.block == b.first)
                    continue;
                auto &verts = candidateVertices[blockIdx];
                if (inside(c, bo.min, bo.max))
                    verts.push_back(v);
            }
        }

        for (Index blockIdx = 0; blockIdx < allBounds.size(); ++blockIdx) {
            b.second.exchangeSize.push_back(candidateVertices[blockIdx].size());
        }
    }

    std::vector<std::vector<ExchangeSize>> send(size()), recv(size());
    for (Index blockIdx = 0; blockIdx < allBounds.size(); ++blockIdx) {
        auto &bo = allBounds[blockIdx];
        int r = bo.rank;
        for (auto &b: m_blockData) {
            if (bo.block == b.first)
                continue;
            send[r].emplace_back(b.first, b.second.exchangeSize[blockIdx]);
        }
    }
    ...
    for (int i = 0; i < size(); ++i) {
        int rp = (rank() + i) % size();
        int rm = (rank() + size() - i) % size();
        if (rp != rank())
            requests.emplace_back(comm().isend(rp, rp, send[rp]));
        if (rm != rank())
            requests.emplace_back(comm().irecv(rm, rank(), recv[rm]));
    }
    ...
    for (auto &s: send) {
        for (auto &e: s)
            sendInfo("block %d: to send %ld", e.block, (unsigned long)e.nvert);
    }
    for (auto &r: recv) {
        for (auto &e: r)
            sendInfo("block %d: to recv %ld", e.block, (unsigned long)e.nvert);
    }
